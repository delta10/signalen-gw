...
def buitenbeter():
    ...
    response = requests.post(url, data=json.dumps(data), headers={ **headers, **app.config['ADDITIONAL_SIGNALEN_HEADERS'] }, timeout=5)
    ...
    if bijlage:
        ...
        response = requests.post(url, data=data, files=files, headers=app.config['ADDITIONAL_SIGNALEN_HEADERS'], timeout=5)
        if not response.ok:
